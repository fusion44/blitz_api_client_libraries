...
class Statistics {
  ...
  @override
  String toString() => 'Statistics[period=$period, threshold=$threshold, elapsed=$elapsed, count=$count, possible=$possible]';
  ...
  static Map<String, List<Statistics>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Statistics.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
